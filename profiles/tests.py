...
class ProfileTestCase(TestCase):
    ...
    def test_following(self):
        first = self.user
        second = self.userb
        first.profile.followers.add(second) # added a follower. Added second user to first user follower
        second_user_following_who = second.following.all()
        qs = second_user_following_who.filter(user=first) # form a user, ckeck if the other user is being followed 
        first_user_following_no_one = first.following.all() # checking if new user is not following anyone
